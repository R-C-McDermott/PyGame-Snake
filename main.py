...
W_MULT = 32
H_MULT = 24
WIDTH, HEIGHT = W_MULT*SNAKE_WIDTH, H_MULT*SNAKE_HEIGHT
...
def draw_game(snake, fruit, snake_body):
    WIN.fill(BACKGROUND)
...
def snake_move(snake, key, snake_body):
    ...
    if (key[pygame.K_UP] and snake.y > 0 and
        ...
        snake_change_x = 0
    if (key[pygame.K_DOWN] and snake.y < HEIGHT - SNAKE_HEIGHT and
            (snake.y + SNAKE_WIDTH) not in [snake_list[1] for snake_list in snake_body]):
        snake_change_y = SNAKE_HEIGHT
        snake_change_x = 0
...
def main():
    ...
    while run:
        ...
        draw_game(snake, fruit, snake_body)
        ...
        if snake.y < 0 or snake.y > (HEIGHT - SNAKE_HEIGHT):
            run = False
            WIN.blit(LOSE_SCREEN, (0, 0))
            pygame.display.update()
            time.sleep(3)
